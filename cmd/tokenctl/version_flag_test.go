...
// prints the bare release tag (`tokenctl v0.15.0`) so release-pinning scripts
...
func TestVersionFlag(t *testing.T) {
	...
	t.Run("version subcommand", func(t *testing.T) {
		...
		got := out.String()
		if !strings.HasPrefix(got, "tokenctl v0.15.0") {
			t.Fatalf("version subcommand output = %q, want prefix %q", got, "tokenctl v0.15.0")
		}
...
func TestVersionConstantPinsReleaseTag(t *testing.T) {
	if Version != "v0.15.0" {
		t.Fatalf("main.Version = %q, want %q (bump in lock-step with VERSION)", Version, "v0.15.0")
	}
}
